...
def main():
    ...
    with open('iot-tweets-2009-2016.tsv', encoding='utf8') as tsvfile, open("iot-tweets-2009-2016-completv3.tsv", "w",
        ...
        w.writerow(
            ['TweetID', 'Sentiment', 'TopicID', 'Country', 'Gender', 'URLs', 'Text', 'User_ID', 'User_Name', 'Date',
             'Hashtags', 'Indication'])
        ...
        for row in reader:
            ...
            if idl == idr:
                line = [idr, row['Sentiment'], row['TopicID'], row['Country'], row['Gender'], row['URLs'],
                        leftFile['tweets'][indexLeftFile]['text'], leftFile['tweets'][indexLeftFile]['user_id'],
                        leftFile['tweets'][indexLeftFile]['user_name'], leftFile['tweets'][indexLeftFile]['date'],
                        leftFile['tweets'][indexLeftFile]['hashtags'], leftFile['tweets'][indexLeftFile]['indication']]
                w.writerow(line)
                counter += 1
                # print ('tweet trouvé')
                indexLeftFile += 1
